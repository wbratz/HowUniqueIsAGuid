...
export default function WitnessCalculator() {
  ...
  const result = useMemo(() => {
    try {
      ...
      if ([Rw, Ry, D].some((x) => !Number.isFinite(x.toNumber()) || x.isNegative()))
        throw new Error('bad')
      ...
      return {
        nGlobal,
        lambda,
        pAnyGlobalText: pAnyGlobal.lessThan(1e-6)
          ? pAnyGlobal.toExponential(2) + '%'
          : pAnyGlobal.toFixed(6) + '%',
        pYouText: pYouWitness.lessThan(1e-6)
          ? pYouWitness.toExponential(2) + '%'
          : pYouWitness.toFixed(6) + '%',
      }
    } catch {
      return null
    }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., 1e12, 1,000,000,000,000"
              ...
              <div>
                ...
                <input
                  value={rateYou}
                  onChange={(e) => setRateYou(e.target.value)}
                  placeholder="e.g., 1e7, 10,000,000"
                  className="mt-1 w-full rounded-xl bg-white/5 px-4 py-3 outline-none ring-1 ring-white/10 focus:ring-accent/50"
                />
              </div>
              <div>
              ...
              <div className="space-y-4">
                <Slider
                  label="What fraction do you actually see?"
                  value={visibility}
                  onChange={setVisibility}
                  suffix="Logs, traces, or IDs you personally inspect vs. what your org generates."
                />
                <Slider
                  label="If both pass through you, chance you recognize it"
                  value={recognition}
                  onChange={setRecognition}
                  suffix="Recognition depends on alerting, dedup checks, humans noticing, etc."
                />
              </div>
            </div>
          </div>
              ...
              <>
                ...
                <div className="mt-4 border-t border-white/10 pt-4">
                  <p className="text-sm text-white/70">
                    Chance <span className="font-semibold">you</span> witness & recognize ≥1
                  </p>
